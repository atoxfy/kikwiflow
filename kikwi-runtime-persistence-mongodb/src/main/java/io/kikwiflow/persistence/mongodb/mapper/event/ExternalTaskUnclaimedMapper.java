...
public final class ExternalTaskUnclaimedMapper {

    private ExternalTaskUnclaimedMapper() {}

    public static Document toDocument(ExternalTaskUnclaimed event) {
        return new Document("externalTaskId", event.externalTaskId())
                ...
                .append("previousAssignee", event.previousAssignee())
                .append("actorId", event.actorId())
                .append("unclaimedAt", event.unclaimedAt() != null ? java.util.Date.from(event.unclaimedAt()) : null);
    }

    public static ExternalTaskUnclaimed fromDocument(Document doc) {
        return new ExternalTaskUnclaimed(
                ...
                doc.getString("previousAssignee"),
                doc.getString("actorId"),
                InstantMapper.mapToInstant("unclaimedAt", doc)
        );
    }
}
